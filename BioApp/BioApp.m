...
@implementation BioApp
-(NSString*)toString:(BOOL)addNameWrap {
    NSMutableString *nsString = [NSMutableString string];
    if (addNameWrap == YES)
        [nsString appendString:@"<BioApp>" ];
    [nsString appendFormat:@"<SoapVersion>%@</SoapVersion>" , [BioApp_Enums SoapProtocolVersionToString:[self soapVersion]]];
...
-(void)CalculateSize:(NSString *)gender :(NSString *)cm :(NSString *)age :(NSString *)weight :(NSString *)height :(NSString *)collarsize :(NSString *)fit :(NSString *)email :(NSString *)fCup :(NSString *)fArms :(NSString *)fWaist :(NSString *)fHips {
    ...
    [params appendString:@"<CalculateSize xmlns=\"http://tempuri.org/\">"];
    [params appendFormat:@"<Gender>%@</Gender>" , gender];
    [params appendFormat:@"<Cm>%@</Cm>" , cm];
    [params appendFormat:@"<Age>%@</Age>" , age];
    [params appendFormat:@"<Weight>%@</Weight>" , weight];
    [params appendFormat:@"<Height>%@</Height>" , height];
    [params appendFormat:@"<Collarsize>%@</Collarsize>" , collarsize];
    [params appendFormat:@"<Fit>%@</Fit>" , fit];
    [params appendFormat:@"<Email>%@</Email>" , email];
    [params appendFormat:@"<FCup>%@</FCup>" , fCup];
    [params appendFormat:@"<FArms>%@</FArms>" , fArms];
    [params appendFormat:@"<FWaist>%@</FWaist>" , fWaist];
    [params appendFormat:@"<FHips>%@</FHips>" , fHips];
    [params appendString:@"</CalculateSize>"];
    ...
    [onwreq addValue:@"http://tempuri.org/CalculateSize" forHTTPHeaderField:@"SOAPAction"];
    ...
    [self setEventName:@"http://tempuri.org/CalculateSizeDidFinish"];
}
...
-(id)copyWithZone:(NSZone *)zone {
    BioApp *finalCopy = [[[self class] allocWithZone: zone] init];
...
-(id)init{
    self = [super init];
    if (self){
        [self setUrl:@"http://biometric.bivolino.com:81/csp/div/BioApp.cls?WSDL"];
    }
    return self;
}
